...
const Dashboard = () => {
  ...
  const renderApplauds = (
    applaudQueryResult: ApplaudQueryHookResult,
    showName: boolean
  ) => {
    if (
      applaudQueryResult.data &&
      applaudQueryResult.data.applaud &&
      applaudQueryResult.data.applaud.nodes &&
      applaudQueryResult.data.applaud.nodes.length > 0
    ) {
      return applaudQueryResult.data.applaud.nodes.map(
      ...
      <Grid container xs={12} spacing={3} item>
        <Grid xs={4} item>
          ...
          <Box paddingY={2} paddingX={1} className="scrollable-section">
            {applaudReceivedQuery.loading ? (
              <Loader type={LOADER_TYPE.card} />
            ) : (
              renderApplauds(applaudReceivedQuery, false)
            )}
          </Box>
        </Grid>
        <Grid xs={4} item>
          ...
          <Box paddingY={2} paddingX={1} className="scrollable-section">
            {applaudGivenQuery.loading ? (
              <Loader type={LOADER_TYPE.card} />
            ) : (
              renderApplauds(applaudGivenQuery, true)
            )}
          </Box>
        </Grid>
        <Grid xs={4} item>
          ...
          <Box paddingY={2} paddingX={1} className="scrollable-section">
            {applaudStreamQuery.loading ? (
              <Loader type={LOADER_TYPE.card} />
            ) : (
              renderApplauds(applaudStreamQuery, true)
            )}
          </Box>
        </Grid>
      </Grid>
